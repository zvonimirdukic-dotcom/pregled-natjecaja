"""Public procurement monitor. Never treats an unreadable source as zero results."""
...
REGION = re.compile(r"slavonski brod|nova gradiska|pozeg|brodsko.posav|hr023|hr024")
...
def signature(item):
    data = json.dumps(item, ensure_ascii=False, sort_keys=True).encode()
    return hashlib.sha256(data).hexdigest()


def source_status(source, status, count=0, detail=""):
    return dict(source=source, status=status, count=count, detail=detail)
...
def parse_eojn_row(headers, cells, links, source):
    ...
    if not title or not buyer or not url:
        raise ValueError("EOJN redak nema predmet, naručitelja ili trajni link")
    return dict(
        source=source,
        buyer=buyer,
        title=title,
...
def collect_eojn(since, max_pages=400):
    ...
    results, coverage = [], []
    feeds = [
        ("Postupci (javna i jednostavna nabava)", "procurements-all", "NoticePublishDate"),
        ("Objave i izmjene", "notices-all", "PublishDate"),
    ]
    with sync_playwright() as p:
        ...
        for label, path, field in feeds:
            ...
            try:
                ...
                headers = [
                    h.strip()
                    for h in page.locator('[role="columnheader"]').all_text_contents()
                ]
                ...
                for page_no in range(max_pages):
                    ...
                    match = re.search(
                        r"Stranica\s+(\d+)\s+od\s+(\d+)\s+\(([\d., ]+)\s+stav",
                        nav,
                    )
                    if not match:
                        raise ValueError("Nije potvrđen broj stranica EOJN-a")
                    current = int(match[1])
                    total = int(match[2])
                    ...
                    if expected is None:
                        ...
                        raise ValueError(
                            "Broj objava promijenio se tijekom pregleda; ponoviti pregled"
                        )

                    rendered = page.locator("tr.dx-data-row").evaluate_all("""
                        els => els.map(e => ({
                    ...
                    if fingerprint in visited and count:
                        raise ValueError("Ponavlja se ista stranica rezultata")
                    ...
                    for raw in rendered:
                        ...
                        if not item["published"] or item["published"] < since:
                            raise ValueError(
                                "Datum EOJN retka ne odgovara traženom razdoblju"
                            )
                        rows_read += 1
                    ...
                    old_rows = page.locator("tr.dx-data-row").all_text_contents()
                    page.get_by_role(
                    ...
                    raise ValueError(f"Dosegnut limit {max_pages} stranica")

                coverage.append(source_status(
                    name, "PREGLEDANO", rows_read,
                    f"Razdoblje od {since}; svi retci filtriranog popisa"
                ))
            except Exception as e:
                coverage.append(source_status(
                    name, "NEPOTPUNO", rows_read,
    ...
    with session.get(url, timeout=(12, 25), stream=True) as r:
        ...
        if "pdf" in mime or content.startswith(b"%PDF"):
            ...
            if len(text.strip()) < 50:
                raise ValueError("PDF bez čitljivog teksta; potreban OCR")
        ...
        if urlparse(final).path.lower().endswith((".doc", ".xls", ".xlsx", ".zip")):
            raise ValueError("Privitak zahtijeva dodatni čitač; provjeriti izvor")
        if "html" not in mime and not content.lstrip().lower().startswith(
            ...
            try:
                ...
                if soup:
                    ...
                    if not title or norm(title) in (
                        "procitajte vise", "saznajte vise", "vise", "detalji"
                    ):
                        title = unquote(
                            urlparse(final).path.rstrip("/").rsplit("/", 1)[-1]
                        ).replace("-", " ")
                title = (title or source["name"])[:350]
                procurement = bool(PROC.search(norm(title + " " + url)))

                date_value = ""
                if soup:
                    dated = soup.select_one(
                ...
                if (
                    ...
                    and not re.search(
                        r"pravilnik|plan nabave|registar ugovor", norm(title)
                    )
                ):
                    generic = norm(title).strip() in (
                        "javna nabava", "jednostavna nabava", "nabava",
                        "natjecaji", "novosti", "postupci javne nabave"
                    )
                    recent = (
                        not date_value
                        or date_value >= (today - timedelta(days=30)).isoformat()
                    )
                    if url != start and not generic and recent:
                        evidence_match = PRODUCT.search(norm(text))
                        pos = evidence_match.start() if evidence_match else 0
                        evidence = re.sub(
                            r"\s+", " ", text[max(0, pos - 35):pos + 100]
                        )
                        found[final] = dict(
                            source=name, buyer=name, title=title, url=final,
                            published=date_value, deadline="", value="", cpv="",
                            reference="",
                            kind="Moguća nabava — provjeriti dokumentaciju",
                            status="",
                            region=source.get("region", "Područje praćenja"),
                            evidence=evidence,
                        )
                ...
                if soup:
                    ...
                    for target, meta in links.items():
                        target_domain = (
                            urlparse(target).netloc.lower().removeprefix("www.")
                        )
                        if target_domain != domain:
                            continue
                        if target not in seen and target not in [q[0] for q in queue]:
                            queue.append((target, meta["title"], meta["context"]))
            except Exception as e:
                errors.append(
                    f"{url}: {type(e).__name__}: {str(e).splitlines()[0][:100]}"
                )

        if queue:
            errors.append(
                f"Preostalo {len(queue)} poveznica; dosegnut vremenski/brojčani limit"
            )

    if len(seen) <= 1 and not found:
        errors.append("Nije potvrđen zaseban popis nabave; potrebna provjera adrese")
    state = (
        "NEPOTPUNO" if errors
...
def build_report(items, coverage, previous, today, initial):
    unique = {}
    for item in items:
        key = item["url"] + "|" + item["reference"] + "|" + item["kind"]
        unique[key] = item
    ...
    changes.sort(key=lambda i: (
        not bool(REGION.search(norm(i["buyer"] + " " + i["region"]))),
        i["published"],
    ))
    failures = sum("NEPOTPUNO" in c["status"] for c in coverage)
    lines = [
        f"Pregled natječaja — {today:%d.%m.%Y.}",
        f"Novih/izmijenjenih kandidata: {len(changes)}. "
        f"Izvora s nepotpunim pregledom: {failures}.",
        "Obuhvat: EOJN za cijelu RH; mrežne stranice iz registra za dvije županije.",
        "Registar ustanova još nije potvrđen kao iscrpan. PREGLEDANO na mrežnoj "
        "stranici znači samo da su obrađene otkrivene poveznice.",
        "",
    ]
    if initial:
        lines += [
            "Prvo pokretanje: početni pregled; objave bez potvrđenog datuma "
            "nisu označene kao objavljene danas.",
            "",
        ]
    if not changes:
        lines += [
            "Nema novih automatski prepoznatih kandidata. To nije potvrda da "
            "na nepročitanim izvorima nema natječaja.",
            "",
        ]
    ...
    for item in changes:
        lines += [
            ...
            f"Vrsta/status: {item['kind']} / {item['status'] or 'nije potvrđen'}",
            f"Objavljeno: {item['published'] or 'datum nije potvrđen'}",
            f"Rok: {item['deadline'] or 'provjeriti u dokumentaciji'}",
            f"Procijenjena vrijednost (EUR): {item['value'] or 'nije potvrđena'}",
    ...
    for c in coverage:
        lines.append(
            f"{c['status']} | {c['source']} | {c['count']} stavki/stranica | "
            f"{c['detail']}"
        )
    lines += [
        "",
        "Izdvajanje koristi ključne riječi i CPV. Skenirani PDF-ovi, nepodržani "
        "privitci, zatvorene objave i stavke bez prepoznatljivog naslova "
...
def main():
    ...
    now, out = datetime.now(TZ), ROOT / "reports"
    ...
    if state.get("last_sent"):
        since = (
            date.fromisoformat(state["last_sent"]) - timedelta(days=2)
        ).isoformat()
    ...
    if not args.skip_eojn:
        try:
            ...
            coverage = [source_status(
                "EOJN", "NEPOTPUNO",
                detail=type(e).__name__ + ": " + str(e).splitlines()[0][:150]
            )]
    else:
        coverage.append(source_status(
            "EOJN", "NEPOTPUNO", detail="Preskočen radi provjere drugih izvora"
        ))
    ...
    with ThreadPoolExecutor(max_workers=6) as pool:
        for found, status in pool.map(
            ...
            print(
                f"{status['source']}: {status['status']} ({status['count']})",
                flush=True,
            )

    body, changes, seen = build_report(
        items, coverage, state.get("seen", {}), now.date(), initial
    )
    (out / "izvjestaj.txt").write_text(body, encoding="utf-8")
    ...
    (out / "rezultati.json").write_text(
        json.dumps(
            dict(generated=now.isoformat(), changes=changes, coverage=coverage),
    ...
    if args.send:
        if args.skip_eojn or args.source_limit:
            raise RuntimeError("Slanje ograničenog testnog pregleda nije dopušteno")
        sender = os.getenv("SMTP_USER")
        recipient = os.getenv("REPORT_TO")
        password = os.getenv("SMTP_PASSWORD")
        if not all((sender, recipient, password)):
            raise RuntimeError(
                "Izvještaj spremljen, ALI NIJE POSLAN: nedostaju SMTP postavke "
                "/ GMAIL_APP_PASSWORD"
            )

        msg = EmailMessage()
        incomplete = any("NEPOTPUNO" in c["status"] for c in coverage)
        msg["Subject"] = (
        ...
        with smtplib.SMTP_SSL("smtp.gmail.com", 465, timeout=45) as smtp:
            ...
            if refused:
                raise RuntimeError("Poslužitelj nije prihvatio primatelja")

        state_path.parent.mkdir(exist_ok=True)
        updated = dict(seen=seen, last_sent=state.get("last_sent"))
        if all(
            c["status"] == "PREGLEDANO"
            for c in coverage if c["source"].startswith("EOJN")
        ):
